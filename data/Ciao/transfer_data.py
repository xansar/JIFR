...
def transfer_data():
    ...
    link_names = ['user1', 'user2']
    link_dtype = {'user1': int, 'user2': int}
    link_df = pd.read_csv('./raw_data/trust.txt', delimiter=' ', names=link_names, engine='python', dtype=link_dtype)
    link_df['weight'] = 1

    rate_names = ['user', 'item', 'rate']
    rate_dtype = {'user': int, 'item': int, 'rate':int}
    rate_df = pd.read_csv('./raw_data/rating.txt', delimiter=' ', names=rate_names, usecols=[0, 1, 3], engine='python', dtype=rate_dtype)
    # link_df = link_df[link_df.weight==1]
    if not os.path.isdir('./behavior_data/'):
