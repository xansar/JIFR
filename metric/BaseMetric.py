...
class BaseMetric:
    ...
    def compute_metrics(self, batch_users, rating_k, gt, task):
        ...
        self._compute_Recall(batch_users, mask, task, gt=gt)
        self._compute_nDCG(batch_users, mask, task, iDCG_mask=iDCG_mask)
        self._compute_Precision(batch_users, mask, task)
        # for m in self.metric_name:
    ...
    def _compute_Precision(self, batch_users, mask, task, **kwargs):
        for k in self.ks:
            mask_k = mask[:, :k]
    ...
    def _compute_Recall(self, batch_users, mask, task, **kwargs):
    ...
    def _compute_nDCG(self, batch_users, mask, task, **kwargs):
