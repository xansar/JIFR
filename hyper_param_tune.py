...
def objective(trial):
    ...
    config['TRAIN']['random_seed'] = str(random.randint(0, 999999))
    global SEED
    SEED = eval(config['TRAIN']['random_seed'])
    # 学习率只衰减不重启
    config['OPTIM']['T_0'] = config['TRAIN']['epoch']
    # 调参时不打印训练信息
    config['LOG'] = False
    with HiddenPrints():
...
def search(model_name, n_trials):
    global SEED
...
def visulization(study, model_name, dataset_name, save_dir=None):
    funcs = [
        'param_importances',
        'parallel_coordinate',
        'intermediate_values',
    ...
    save_pth = os.path.join(save_dir, model_name, dataset_name)
    for func in funcs:
...
if __name__ == '__main__':
    ...
    n_trials = 2
    study = search(model, n_trials)
    visulization(study, model_name=model, dataset_name=dataset, save_dir='./params_search')
    print(
